...
def plot_campaign_pnl(campaign_data: Dict) -> go.Figure:
    ...
    """
    realized = campaign_data.get("realized_pnl", 0)
    ...
    fig.update_layout(
        title=f"{campaign_data['symbol']} P&L Breakdown",
        yaxis_title="P&L ($)",
        template="plotly_dark"
    )
...
def plot_combined_pnl(transactions: List[Dict], holdings: Dict[str, Dict]) -> go.Figure:
    """
    绘制综合盈亏图（Realized + Unrealized）
    """
    fig = go.Figure()

    # Realized P&L
    symbols = list(holdings.keys())
    realized = [holdings[s].get("realized_pnl", 0) for s in symbols]
