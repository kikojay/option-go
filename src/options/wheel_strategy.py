...
class WheelStrategyCalculator:
    ...
    def calculate_adjusted_cost_basis(self, symbol: str) -> Dict:
        ...
        Returns:
            {
                "current_shares": 当前持仓数,
                "adjusted_cost": 调整后成本(每股),
                "cost_basis": 调整后成本基准(总额),
                "total_premiums": 权利金累计,
                "premiums_per_share": 每股权利金,
                "fees_per_share": 每股手续费,
                "option_positions": 当前期权仓位
            }
        """
    ...
    def calculate_breakeven_weeks(self, symbol: str, avg_weekly_premium: float, 
        ...
        Args:
            symbol: 股票代码
            avg_weekly_premium: 平均每周收集的权利金
            target_cost: 目标成本（默认0，表示完全回本）
